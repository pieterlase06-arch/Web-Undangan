...
const Editor = ({ data, updateData, setView: setViewProp }) => {
  ...
  // If setViewProp is not provided, use the local navigate function
  ...
  const renderDesignTab = () => (
    <div className="space-y-6">
       <section className="space-y-4">
          <p className="text-[9px] font-black uppercase tracking-widest text-stone-500">Gaya & Warna</p>
          <div className="grid grid-cols-1 gap-4">
             <div className="space-y-2">
                <label className="text-[9px] font-black text-stone-400 uppercase">Warna Utama</label>
       ...
       <section className="space-y-4 pt-6 border-t border-stone-100">
          ...
          <p className="text-[8px] opacity-40 italic px-2 text-stone-600">Gunakan link langsung (.mp3) agar musik bisa diputar otomatis.</p>
       </section>
    </div>
  );
    ...
    <div className="space-y-8 pb-10">
             ...
             <div className="p-4 bg-stone-50 rounded-2xl space-y-4 border border-stone-100 shadow-sm">
                <div className="flex items-center gap-4">
                   ...
                   <div className="flex-1 space-y-2">
                      ...
                      <input className="w-full bg-transparent py-1 text-[10px] text-stone-500 font-bold outline-none" placeholder="Nama Orang Tua Pria" value={data.partner1Parents} onChange={(e) => updateData({ partner1Parents: e.target.value })} />
                   </div>
                </div>
             </div>
             <div className="p-4 bg-stone-50 rounded-2xl space-y-4 border border-stone-100 shadow-sm">
                <div className="flex items-center gap-4">
                   ...
                   <div className="flex-1 space-y-2">
                      ...
                      <input className="w-full bg-transparent py-1 text-[10px] text-stone-500 font-bold outline-none" placeholder="Nama Orang Tua Wanita" value={data.partner2Parents} onChange={(e) => updateData({ partner2Parents: e.target.value })} />
                   </div>
                </div>
             </div>
          </div>
       ...
       <section className="space-y-4 pt-6 border-t border-stone-100">
          ...
          <div className="p-6 bg-stone-100/50 rounded-3xl space-y-6 border border-stone-100">
             <div className="space-y-2">
                <label className="text-[9px] font-black text-stone-400 uppercase">Tanggal & Waktu</label>
                <div className="grid grid-cols-2 gap-3">
                   ...
                   <input type="text" className="w-full p-3 bg-white border border-stone-200 rounded-xl outline-none text-xs font-bold text-stone-900" placeholder="08:00 WIB" value={data.time} onChange={(e) => updateData({ time: e.target.value })} />
                </div>
             </div>
             <div className="space-y-2">
             ...
             <div className="space-y-2">
                <label className="text-[9px] font-black text-stone-400 uppercase">Alamat Lengkap</label>
                <textarea className="w-full p-3 bg-white border border-stone-200 rounded-xl outline-none text-xs font-bold h-20 text-stone-900" placeholder="Jl. Raya..." value={data.address} onChange={(e) => updateData({ address: e.target.value })} />
             </div>
             <div className="space-y-2">
                <label className="text-[9px] font-black text-stone-400 uppercase">Google Maps Link</label>
                <input className="w-full p-3 bg-white border border-stone-200 rounded-xl outline-none text-[9px] font-mono text-stone-500" placeholder="https://..." value={data.mapsLink} onChange={(e) => updateData({ mapsLink: e.target.value })} />
             </div>
          </div>
       </section>

       <section className="space-y-4 pt-6 border-t border-stone-100">
          <p className="text-[9px] font-black uppercase tracking-widest text-stone-500">Kutipan / Doa</p>
          <textarea className="w-full p-6 bg-stone-50 rounded-3xl outline-none text-xs italic border border-stone-200 h-32 text-stone-900" placeholder="Doa restu..." value={data.quote} onChange={(e) => updateData({ quote: e.target.value })} />
       </section>

       <section className="space-y-4 pt-6 border-t border-stone-100">
          <div className="flex items-center justify-between">
             <p className="text-[9px] font-black uppercase tracking-widest text-stone-500">Cerita Cinta</p>
             <button onClick={addStory} className="text-[9px] font-black text-[#C5A059]">+ TAMBAH</button>
          </div>
          <div className="space-y-4">
             {(data.stories || []).map((s, i) => (
               <div key={i} className="p-4 bg-white border border-stone-200 rounded-2xl space-y-3 relative group">
                  <button onClick={() => updateData({ stories: data.stories.filter((_, idx) => idx !== i) })} className="absolute top-4 right-4 opacity-0 group-hover:opacity-100 text-red-500 transition-opacity">
                     <span className="material-symbols-outlined text-[16px]">delete</span>
                  </button>
                  <div className="grid grid-cols-2 gap-2">
                     <input className="bg-stone-50 p-2 rounded-lg text-[9px] font-black text-stone-900 outline-none" placeholder="Tahun" value={s.year} onChange={(e) => { const st = [...data.stories]; st[i].year = e.target.value; updateData({ stories: st }); }} />
                     <input className="bg-stone-50 p-2 rounded-lg text-[9px] font-black text-stone-900 outline-none" placeholder="Icon" value={s.icon} onChange={(e) => { const st = [...data.stories]; st[i].icon = e.target.value; updateData({ stories: st }); }} />
                  </div>
                  <input className="w-full bg-transparent border-b border-stone-100 py-1 text-xs font-black text-stone-900 outline-none" placeholder="Judul" value={s.title} onChange={(e) => { const st = [...data.stories]; st[i].title = e.target.value; updateData({ stories: st }); }} />
               </div>
             ))}
          </div>
       </section>
  ...
  return (
    <div className="h-screen flex flex-col overflow-hidden bg-white">
      <header className="h-14 border-b px-6 flex items-center justify-between z-[100] bg-white border-stone-100">
        <button onClick={() => setView('/desain-saya')} className="text-stone-400 hover:text-stone-900 flex items-center gap-3 group">
          <div className="w-8 h-8 rounded-lg bg-stone-50 group-hover:bg-stone-100 flex items-center justify-center transition-all">
        ...
        <div className="flex items-center gap-6">
          {isUploading && <div className="flex items-center gap-2"><div className="w-2 h-2 bg-[#C5A059] rounded-full animate-ping" /><span className="text-[9px] font-black text-[#C5A059]">SAVING...</span></div>}
          <button onClick={() => setIsPublished(true)} className="px-6 py-2 bg-stone-950 text-white rounded-xl text-[10px] font-black uppercase tracking-[0.2em] shadow-lg hover:bg-[#C5A059]">Publish</button>
        </div>
      </header>

      <div className="flex flex-1 overflow-hidden relative">
        <aside className="w-[360px] border-r flex flex-col h-full bg-white border-stone-50 z-10 shadow-sm">
        ...
        <main className="flex-1 relative overflow-hidden bg-[#f4f6f8] flex items-center justify-center p-8 lg:p-12">
      ...
      <AnimatePresence>
         {isPublished && (
           <div className="fixed inset-0 z-[2000] flex items-center justify-center p-6 bg-stone-900/80 backdrop-blur-xl">
              <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="max-w-md w-full p-12 bg-white rounded-[60px] text-center shadow-2xl border border-white">
                 ...
                 <div className="bg-stone-50 p-6 rounded-3xl mb-10 text-[10px] font-mono break-all text-[#C5A059] flex items-center justify-between border border-stone-100">
                    ...
                    <button onClick={() => { navigator.clipboard.writeText(config.BASE_URL+'#/v'); alert('Link Tersalin!'); }} className="material-symbols-outlined text-[18px]">content_copy</button>
                 </div>
