  ...
  return (
    <div className={`h-screen flex flex-col overflow-hidden transition-colors duration-300 ${theme === 'dark' ? 'bg-[#0f172a]' : 'bg-[#fcf9f6]'}`}>
      ...
      <div className="flex flex-1 overflow-hidden relative">
        {/* LEFT TOOLBAR - ICON ACCESS */}
        <aside className={`w-16 border-r flex flex-col items-center py-6 gap-6 z-40 transition-colors ${theme === 'dark' ? 'bg-[#1e293b] border-slate-700' : 'bg-white border-stone-100'}`}>
        ...
        {/* MAIN CANVAS */}
        <main className="flex-1 relative overflow-hidden flex items-center justify-center p-12 bg-stone-50/50 dark:bg-slate-950/20">
          <div className={`absolute bottom-8 left-1/2 -translate-x-1/2 floating-toolbar px-5 py-2 rounded-full shadow-lg backdrop-blur-md flex items-center gap-4 z-30 border ${theme === 'dark' ? 'bg-slate-800/80 border-slate-700' : 'bg-white/80 border-stone-100'}`}>
          ...
          >
             ...
             <div className="relative z-10 flex flex-col items-center text-center gap-8 w-full">
               ...
               <h1 className="text-[48px] text-stone-900 leading-[1] relative font-serif" style={{ color: data.primaryColor }}>
                 {data.partner1}<br/><span className="text-[32px] italic mx-4 font-light" style={{ color: data.accentColor }}>&</span><br/>{data.partner2}
               </h1>
               ...
               <div className="text-stone-400 text-[13px] leading-relaxed italic">{data.venue}<br/>{data.address}</div>
             </div>
          </motion.div>
        </main>

        {/* RIGHT SETTINGS PANEL */}
        <aside className={`w-[350px] border-l flex flex-col h-full z-40 overflow-hidden transition-colors ${theme === 'dark' ? 'bg-[#1e293b] border-slate-700' : 'bg-white border-stone-100'}`}>
          ...
          <div className="flex-1 overflow-y-auto p-8 custom-scrollbar">
            <AnimatePresence mode="wait">
              ...
              {activeTab === 'content' && (
                <motion.div key="content" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-10">
              ...
              {activeTab === 'layers' && (
                <motion.div key="layers" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-4">
                   {[
                     ...
                     <div key={layer.id} className={`flex items-center justify-between p-4 rounded-2xl border group cursor-pointer hover:translate-x-1 transition-all ${theme === 'dark' ? 'bg-slate-800/50 border-slate-700 hover:bg-slate-800' : 'bg-stone-50/50 border-stone-100 hover:bg-stone-50'}`}>
                        ...
                        <span className={`material-symbols-outlined text-[18px] ${layer.status === 'Hidden' ? 'text-stone-300 opacity-20' : 'text-stone-300 group-hover:text-stone-500'}`}>{layer.status === 'Hidden' ? 'visibility_off' : 'visibility'}</span>
                     </div>
                   ))}
                </motion.div>
              )}
      ...
      <AnimatePresence>
        {isPreviewOpen && (
          ...
          >
            <div className={`h-16 flex items-center justify-between px-10 border-b sticky top-0 z-[1100] ${theme === 'dark' ? 'bg-slate-900/80 border-slate-800' : 'bg-white/80 border-stone-100'} backdrop-blur-md`}>
